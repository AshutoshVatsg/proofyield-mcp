...
export function hashWithout<T extends Record<string, unknown>>(value: T, ...keys: string[]): `0x${string}` {
...
export function refreshReceiptHash(receipt: DecisionReceipt): DecisionReceipt {
  const updatedAt = new Date().toISOString();
  const next = { ...receipt, updatedAt, finalReceiptHash: `0x${'0'.repeat(64)}` } as DecisionReceipt;
  next.finalReceiptHash = hashWithout(next as unknown as Record<string, unknown>, 'finalReceiptHash');
