...
export class PlanningTools {
  ...
  async createPlan(input: z.infer<typeof CreatePlanInputSchema>, context: ExecutionContext): Promise<DecisionReceipt> {
    ...
    const expectedSnapshotHash = hashWithout(snapshot as unknown as Record<string, unknown>, 'snapshotHash');
    if (snapshot.snapshotHash !== expectedSnapshotHash) throw new Error('Snapshot hash mismatch; refresh the wallet snapshot.');
    if (!this.evidenceRepository.matchesSnapshot(snapshot)) throw new Error('Snapshot was not issued by this ProofYield server; refresh discovery evidence.');
    ...
    const eligible: z.infer<typeof EligibleCandidateSchema>[] = [];
    const deployAmounts = new Map<string, { decimals: number; baseUnits: bigint; formatted: string; reserve: string }>();
    const nowMs = Date.now();

    for (const opportunity of opportunities) {
      const reject = (code: RejectionResult['code'], reason: string) => {
      ...
      if (opportunity.protocolId === 'do-nothing') {
        ...
        const walletAsset = chainSnapshot.assets.find((asset) =>
          asset.symbol === policy.assetSymbol
      ...
      try {
        for (const position of chainSnapshot.positions.filter((item) => item.assetSymbol === policy.assetSymbol)) {
          ...
          if (position.protocolId === opportunity.protocolId) existingProtocolBaseUnits += positionAmount;
        }
      } catch {
      ...
      const remainingProtocolCapacity = maxProtocolBaseUnits > existingProtocolBaseUnits
        ...
        : 0n;
      const concentrationBound = minBigInt(
        percentage(requestedBaseUnits, policy.maxProtocolPct),
        remainingProtocolCapacity,
      );
    ...
    if (!selected) throw new Error('No safe candidate exists. Refresh data or adjust the policy without weakening address/simulation controls.');
    ...
    const steps: ActionPlan['steps'] = actionType === 'DO_NOTHING'
    ...
    planDraft.planHash = hashWithout(planDraft as unknown as Record<string, unknown>, 'planHash');
    ...
    let receipt = {
      ...
      walletAddress: snapshot.walletAddress,
      intent: { assetSymbol: 'USDC' as const, requestedAmount, horizonDays: policy.horizonDays },
      policy,
      policyHash,
      snapshot,
      opportunities,
  ...
  async simulatePlan(input: { planHash: string }, context: ExecutionContext): Promise<SimulationResult> {
    let receipt = this.requireReceipt(input.planHash);
    const plan = receipt.plan;
    this.validateStoredPlan(receipt);
    if (receipt.stage === 'EXECUTED') throw new Error('This plan has already executed and cannot be replayed.');
    if (receipt.stage === 'FAILED') throw new Error('This plan is closed after a failed execution; create a fresh plan.');

    if (Date.parse(plan.deadline) <= Date.now()) throw new Error('Plan expired; create a fresh plan from current data.');
    ...
    if (!adapter || !chain.rpcUrl || !plan.assetAddress || !plan.targetContract) throw new Error('The plan adapter is no longer configured.');
    if (!this.configService.isTrustedTarget(plan.protocolId, plan.chainId, plan.targetContract)) throw new Error('Plan target is no longer allowlisted.');
    ...
    try {
      const [rpcChainId, block, balanceValue, allowanceValue, targetCode] = await Promise.all([
        ...
        client.readContract({ address: adapter.assetAddress, abi: ERC20_ABI, functionName: 'balanceOf', args: [plan.walletAddress as `0x${string}`] }),
        client.readContract({ address: adapter.assetAddress, abi: ERC20_ABI, functionName: 'allowance', args: [plan.walletAddress as `0x${string}`, adapter.targetAddress] }),
        client.getBytecode({ address: adapter.targetAddress }),
      ]);
      if (rpcChainId !== plan.chainId) throw new Error(`RPC chain ${rpcChainId} does not match plan chain ${plan.chainId}.`);
      blockNumber = block.toString();
      balance = balanceValue;
      allowance = allowanceValue;
      if (!targetCode) throw new Error('Allowlisted target has no bytecode.');
      if (balance < amount) throw new Error('Live USDC balance is below the bounded deployment amount.');
      ...
      if (allowance < amount) {
        ...
        const gas = await client.estimateContractGas({
          ...
          account: plan.walletAddress as `0x${string}`,
        });
        const nextTransaction: TransactionRequest = {
          kind: 'APPROVAL',
          chainId: plan.chainId,
  ...
  async prepareExecution(input: { planHash: string }, _context: ExecutionContext): Promise<PreparedTransaction> {
    let receipt = this.requireReceipt(input.planHash);
    this.validateStoredPlan(receipt);
    if (receipt.stage === 'EXECUTED' || receipt.stage === 'FAILED') throw new Error('This plan is closed and cannot prepare another transaction.');
    const simulation = receipt.simulation;
    if (!simulation || !simulation.nextTransaction) throw new Error('No simulated transaction is available for this plan.');
    if (!['APPROVAL_REQUIRED', 'PASSED'].includes(simulation.status)) throw new Error(`Simulation status ${simulation.status} cannot be prepared.`);
    if (Date.parse(simulation.expiresAt) <= Date.now() || Date.parse(receipt.plan.deadline) <= Date.now()) throw new Error('Simulation or plan expired; simulate again.');
    const expectedSimulationHash = hashWithout(simulation as unknown as Record<string, unknown>, 'simulationHash');
    if (simulation.simulationHash !== expectedSimulationHash) throw new Error('Simulation hash mismatch.');
    ...
    if (!adapter) throw new Error('Adapter is not configured.');
  ...
  async verifyExecution(
    input: { chainId: ChainId; transactionHash: string; planHash: string },
    ...
    if (!prepared) throw new Error('No prepared transaction exists for this plan.');
    if (input.chainId !== prepared.chainId || input.chainId !== decisionReceipt.plan.chainId) throw new Error('Transaction chain does not match the plan.');
    ...
    if (this.receiptRepository.hasVerifiedTransaction(input.transactionHash)) throw new Error('Transaction hash is already bound to another receipt.');

    const chain = this.configService.getChainConfig(input.chainId);
    if (!chain.rpcUrl) throw new Error('RPC is not configured for verification.');
    const client = createPublicClient({ transport: http(chain.rpcUrl, { timeout: 10_000, retryCount: 1 }) });
    const observedTimestamp = new Date().toISOString();

    try {
      const txHash = TransactionHashSchema.parse(input.transactionHash) as `0x${string}`;
      ...
      if (rpcChainId !== input.chainId) throw new Error(`RPC chain ${rpcChainId} does not match verification chain ${input.chainId}.`);
      ...
      const envelopeMatches = Boolean(
        transaction.from.toLowerCase() === prepared.from.toLowerCase()
      ...
      if (!adapter) throw new Error('Adapter configuration was removed before verification.');
      if (prepared.kind === 'APPROVAL') {
        const allowance = await client.readContract({
          address: adapter.assetAddress,
          abi: ERC20_ABI,
          functionName: 'allowance',
          args: [decisionReceipt.plan.walletAddress as `0x${string}`, adapter.targetAddress],
        ...
        const walletBalance = await client.readContract({
          ...
          args: [decisionReceipt.plan.walletAddress as `0x${string}`],
          blockNumber: transactionReceipt.blockNumber,
        });
      ...
      const status: ExecutionVerification['status'] = !receiptSucceeded
        ? 'FAILED'
  ...
  async getReceipt(input: { idOrHash: string }, _context: ExecutionContext): Promise<DecisionReceipt> {
  ...
  async monitorCheck(input: { planHash: string; currentOpportunities: Opportunity[] }, _context: ExecutionContext) {
    ...
    const triggers: Array<{ code: string; reason: string }> = [];
    ...
    if (!selected || selected.status !== 'LIVE') {
      ...
      if (selected.riskScore > receipt.policy.maxRiskScore) triggers.push({ code: 'RISK_LIMIT_EXCEEDED', reason: `Current risk ${selected.riskScore} exceeds policy maximum ${receipt.policy.maxRiskScore}.` });
      if ((Date.now() - Date.parse(selected.provenance.timestamp)) / 1000 > receipt.policy.maxDataAgeSeconds) triggers.push({ code: 'DATA_STALE', reason: 'The selected opportunity data is stale.' });
    }
  ...
  private requireReceipt(idOrHash: string): DecisionReceipt {
    const receipt = this.receiptRepository.get(idOrHash);
    if (!receipt) throw new Error(`Decision Receipt not found: ${idOrHash}`);
    return DecisionReceiptSchema.parse(receipt);
  }

  private validateStoredPlan(receipt: DecisionReceipt): void {
    const planHash = hashWithout(receipt.plan as unknown as Record<string, unknown>, 'planHash');
    if (planHash !== receipt.plan.planHash) throw new Error('Stored plan hash mismatch.');
    if (canonicalHash(receipt.policy) !== receipt.plan.policyHash || receipt.policyHash !== receipt.plan.policyHash) throw new Error('Stored policy hash mismatch.');
    if (receipt.snapshot.snapshotHash !== receipt.plan.snapshotHash) throw new Error('Stored snapshot hash mismatch.');
    if (canonicalHash(receipt.opportunities) !== receipt.plan.opportunitySetHash) throw new Error('Stored opportunity-set hash mismatch.');
    if (!this.configService.isTrustedTarget(receipt.plan.protocolId, receipt.plan.chainId, receipt.plan.targetContract)) throw new Error('Stored target is not currently allowlisted.');
  }

  private async simulateProtocolAction(
    client: ReturnType<typeof createPublicClient>,
    plan: ActionPlan,
    amount: bigint,
  ): Promise<TransactionRequest> {
    const account = plan.walletAddress as `0x${string}`;
    const target = plan.targetContract as `0x${string}`;
    const asset = plan.assetAddress as `0x${string}`;
...
function scoreOpportunity(opportunity: Opportunity, policy: UserPolicy): ScoreBreakdown {
...
function ensureDoNothingCandidate(opportunities: Opportunity[], snapshot: WalletSnapshot, policy: UserPolicy): Opportunity[] {
  if (opportunities.some((opportunity) => opportunity.protocolId === 'do-nothing'
    && opportunity.provenance.sourceId === 'proofyield-policy-baseline')) return opportunities;
  const chain = snapshot.chains.find((item) => item.status === 'LIVE' && policy.allowedChainIds.includes(item.chainId));
  if (!chain) return opportunities;
  const asset = chain.assets.find((item) => item.symbol === 'USDC');
  if (!asset) return opportunities;
...
function percentage(amount: bigint, pct: number): bigint {
...
function finalizeSimulation(value: Omit<SimulationResult, 'simulationHash'>): SimulationResult {
  const draft = { ...value, simulationHash: `0x${'0'.repeat(64)}` } as SimulationResult;
  draft.simulationHash = hashWithout(draft as unknown as Record<string, unknown>, 'simulationHash');
